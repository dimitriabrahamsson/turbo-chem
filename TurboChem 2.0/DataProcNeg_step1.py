...
def readData():
    df = pd.read_excel('NegAreasExp1.xlsx')
...
def qaqc1(df):
    df['W_average'] = np.mean(df.loc[:, 'neg-octlW':'neg_undecW'], axis=1)
    df['W_std'] = np.std(df.loc[:, 'neg-octlW':'neg_undecW'], axis=1)
...
dfe.to_csv('1stExpMergedCleanNeg.csv')
...
df2['BlankCorr'] = df2.loc[:, 'neg_no injection_1':'neg_no injection_4'].mean(axis=1)
